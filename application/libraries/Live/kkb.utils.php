...
class xml {
    ...
    {   
        ...
        xml_set_object($parse, $obj);
	xml_parser_set_option($parse, XML_OPTION_CASE_FOLDING, true);
        xml_set_element_handler($parse, "tag_open", "tag_close");
        xml_set_character_data_handler($parse, "cdata");
    }
	...
	xml_parser_set_option($parse, XML_OPTION_CASE_FOLDING, true);
        ...
        xml_parse($parse, $data);
		ksort($xarray,SORT_STRING);
		return $xarray;
	}
...
// -----------------------------------------------------------------------------------------------
function process_card_payment_request($order_id,$currency_code,$amount,$config_file,$abonent_id,$card_id,$b64=true) {
	// -----===++[Process incoming data to full bank request]++===-----
	// variables:
	// $order_id - integer: order index - recoded to 6 digit format with leaded zero
	// $currency_code - string: preferred currency codes 840-USD, 398-Tenge
	...
	$request['CARD_ID'] = $card_id;
        $request['ABONENT_ID'] = $abonent_id;
	$request['RECEPIENT'] = $config['MERCHANT_ID'];
	//$request['SERVICE_ID'] = '';
	$request['TYPE'] = 1;
	...
	$result = process_XML($config['XML_PAYMENTBYCARD_TEMPLATE_FN'],$request);
	if (strpos($result,"[RERROR]")>0){ return "Error reading XML template.";};
	$result_sign = '<merchant_sign cert_id="'.$config['MERCHANT_CERTIFICATE_ID'].'" type="RSA">'.$kkb->sign64($result).'</merchant_sign>';
	$xml = "<document>".$result.$result_sign."</document>";
	if ($b64){return base64_encode($xml);} else {return $xml;};
};
// -----------------------------------------------------------------------------------------------
function process_addcard_request($order_id,$currency_code,$amount,$config_file,$abonent_id,$b64=true) {
	// -----===++[Process incoming data to full bank request]++===-----
	// variables:
	// $order_id - integer: order index - recoded to 6 digit format with leaded zero
	// $currency_code - string: preferred currency codes 840-USD, 398-Tenge
	...
	$result = process_XML($config['XML_ADDCARD_TEMPLATE_FN'],$request);
	if (strpos($result,"[RERROR]")>0){ return "Error reading XML template.";};
	$result_sign = '<merchant_sign type="RSA">'.$kkb->sign64($result).'</merchant_sign>';
	$xml = "<document>".$result.$result_sign."</document>";
        //echo '<code>' . $xml . '</code>'; die('>>>>>>>>>>>>');
        
	if ($b64){return base64_encode($xml);} else {return $xml;};
};
// -----------------------------------------------------------------------------------------------
function process_delete_get_card_request($config_file,$abonent_id,$card_id,$action,$b64=true) {
	// -----===++[Process incoming data to full bank request]++===-----
	// variables:
	// $order_id - integer: order index - recoded to 6 digit format with leaded zero
	// $currency_code - string: preferred currency codes 840-USD, 398-Tenge
	...
	$result = process_XML($config['XML_DELETECARD_TEMPLATE_FN'],$request);
	if (strpos($result,"[RERROR]")>0){ return "Error reading XML template.";};
	$result_sign = '<merchant_sign cert_id="'.$config['MERCHANT_CERTIFICATE_ID'].'" type="RSA">'.$kkb->sign64($result).'</merchant_sign>';
	$xml = "<document>".$result.$result_sign."</document>";
        return (String) $xml;
...
function process_response($response,$config_file) {
	...
	$xml_parser = new xml();
        ...
        //$result=object2array($xml_object);
